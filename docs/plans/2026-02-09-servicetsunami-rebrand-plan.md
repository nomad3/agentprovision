...
| Old | New |
|-----|-----|
| `AgentProvision` | `ServiceTsunami` |
| `agentprovision` | `servicetsunami` |
| `AGENTPROVISION` | `SERVICETSUNAMI` |
| `agentprovision.com` | `servicetsunami.com` |
| `demo@agentprovision.ai` | `demo@servicetsunami.com` |
| `devops@agentprovision.com` | `devops@servicetsunami.com` |
| `agentprovision_supervisor` (dir) | `servicetsunami_supervisor` |
| `AgentProvisionAPI` (class) | `ServiceTsunamiAPI` |
| `hide_agentprovision_branding` | `hide_servicetsunami_branding` |
| `agentprovision.lang` | `servicetsunami.lang` |

## Execution Steps

### Step 1: Rename files and directories (git mv)
- `apps/adk-server/agentprovision_supervisor/` → `servicetsunami_supervisor/`
- `helm/values/agentprovision-api.yaml` → `servicetsunami-api.yaml`
- `helm/values/agentprovision-web.yaml` → `servicetsunami-web.yaml`
- `helm/values/agentprovision-worker.yaml` → `servicetsunami-worker.yaml`
- `helm/values/agentprovision-adk.yaml` → `servicetsunami-adk.yaml`
- `.github/workflows/agentprovision-api.yaml` → `servicetsunami-api.yaml`
- `.github/workflows/agentprovision-web.yaml` → `servicetsunami-web.yaml`
- `.github/workflows/agentprovision-worker.yaml` → `servicetsunami-worker.yaml`
- `agentprovision.code-workspace` → `servicetsunami.code-workspace`
...
- `kubernetes/gateway/managed-certificate.yaml` — add servicetsunami.com domains
- `kubernetes/ingress.yaml` — update hosts to servicetsunami.com
...
- `apps/adk-server/servicetsunami_supervisor/agent.py` — ADK supervisor definition
...
- `helm/values/servicetsunami-api.yaml` — primary Helm values pattern
- `.github/workflows/servicetsunami-api.yaml` — CI/CD image builds and deploys
...
- Create new GCP secrets with `servicetsunami-*` prefix (copy values from `agentprovision-*`)
- Create `servicetsunami-worker` service account
- Rename database: `ALTER DATABASE agentprovision RENAME TO servicetsunami;`
