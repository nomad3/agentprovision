...
class SkillCredential(Base):
    """Encrypted credential storage for skill configurations."""
    __tablename__ = "skill_credentials"

    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    tenant_id = Column(UUID(as_uuid=True), ForeignKey("tenants.id"), nullable=False, index=True)
    skill_config_id = Column(UUID(as_uuid=True), ForeignKey("skill_configs.id"), nullable=False, index=True)
    ...
    skill_config = relationship("SkillConfig", foreign_keys=[skill_config_id])

    def __repr__(self):
        return f"<SkillCredential {self.credential_key} type={self.credential_type}>"
